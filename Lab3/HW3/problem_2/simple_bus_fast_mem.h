...
simple_bus_status simple_bus_fast_mem::read(int *data
    , unsigned int address)
{
    ...
    unsigned addrX;
    if(ld->read() == false) {
        index = (address-m_start_address)/4;
        addrY = index/m_width;
        addrX = index%m_width;
//cout << "addrY: " << addrY << ", addrX: " << addrX << endl;
        ld->write(true);
        rw->write(READ);
        x->write(addrX);
        y->write(addrY);
    ...
    ld->write(false);
//cout << "before read @ " << sc_time_stamp() << endl;
    *data = d->read().to_int();
//cout << "after read, data = " << *data << endl;
    }
    return SIMPLE_BUS_OK;
}

simple_bus_status simple_bus_fast_mem::write(int *data
    , unsigned int address)
{
    unsigned index;
    unsigned addrY;
