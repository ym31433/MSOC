...
SC_MODULE(simple_bus_test)
{
    ...
    {
        // create instances
        master_b = new simple_bus_master_blocking("master_b", 4, 0x4c, false, 300); // priority, address, lock, timeout
        master_nb = new simple_bus_master_non_blocking("master_nb", 3, 0x38, false, 20); // priority, address, lock, timeout
        master_d = new simple_bus_master_direct("master_d", 0x78, 100); // address, timeout
        mem_fast = new simple_bus_fast_mem("mem_fast", 0x00, 0x7f); // start_address, end_address
        mem_slow = new simple_bus_slow_mem("mem_slow", 0x80, 0xff, 1); // start_address, end_address, period
