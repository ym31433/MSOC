...
void simple_bus_master_blocking::main_action()
{
    ...
    while (true) {
        ...
        status = bus_port->burst_read(m_unique_priority, mydata, m_address, mylength, m_lock); // status: OK, REQUEST, WAIT, ERROR
...
*/
void simple_bus_master_blocking::word_read(unsigned addrY, unsigned addrX, int& data) {
    simple_bus_status status;
    unsigned index = addrY*m_width+addrX;
    status = bus_port->burst_read(m_unique_priority, &data, m_address+index*4, 1, m_lock);
//    if (status == SIMPLE_BUS_ERROR)
//            sb_fprintf(stdout, "%g %s : blocking-read failed at address %x\n", sc_time_stamp().to_double(), name(), m_address);
}

void simple_bus_master_blocking::word_write(unsigned addrY, unsigned addrX, int data) {
    simple_bus_status status;
    unsigned index = addrY*m_width+addrX;
    status = bus_port->burst_write(m_unique_priority, &data, m_address+index*4, 1, m_lock);
//    if (status == SIMPLE_BUS_ERROR)
//            sb_fprintf(stdout, "%g %s : blocking-write failed at address %x\n", 
//            sc_time_stamp().to_double(), name(), m_address);
}
